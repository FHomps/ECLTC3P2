...
class Scene(Serializable):
    ...
    def fromDict(cls, _dict, _raise=True):
        ...
        for s in _dict['sources']:
            if s['_class_'] == 'LightSource':
                scene.append(LightSource.fromDict(s, _raise))
            elif s['_class_'] == 'ConicSource':
                scene.append(ConicSource.fromDict(s, _raise))
            elif s['_class_'] == 'ParallelSource':
                scene.append(ParallelSource.fromDict(s, _raise))
            else:
                scene.append(FiniteConicSource.fromDict(s, _raise))
...
class ConicSource(LightSource):
    def __init__(self, position, direction, halfAngle, color, penumbraSize = 0):
        #Position de l'origine de la source de lumière
        self.position = position
        
        #Direction de la source
        self.direction = direction.norm()
        
        #Demi-angle de la source
        ...
        self.penumbraSize = penumbraSize
    ...
    def get_color(self, M):   
        cosHalfAngle = np.cos(np.pi * self.halfAngle / 180)
        
        penumbraLim = cosHalfAngle - self.penumbraSize
        
        penumbraCoeff = -1
        if self.penumbraSize > 0:
            penumbraCoeff = 1 / self.penumbraSize
    
        def penombre(pscal):
            if penumbraCoeff <= 0:
                return np.greater(pscal, cosHalfAngle)
            temp = (pscal - penumbraLim) * penumbraCoeff
            return np.greater(temp, 1) + np.greater(1, temp) * temp - np.greater(0, temp) * temp
        
        
        Coeff = penombre((M - self.position).norm().dot(self.direction))
        return rgb(Coeff * self.color.x, Coeff * self.color.y, Coeff * self.color.z)
        
    @classmethod
    def keys(cls):
        return ['position', 'direction', 'halfAngle', 'color', 'penumbraSize']
        
class FiniteConicSource(LightSource):
    def __init__(self, position, direction, halfAngle, color, halfLifeDistance, penumbraSize = 0):
        #Position de l'origine de la source de lumière
        self.position = position
        ...
        #Distance de demi-vie de la lumière émise : à cette distance, l'intensité aura été divisée par 2 par rapport à avant.
        self.halfLifeDistance = halfLifeDistance
    ...
    def get_color(self, M):
        ...
        Coeff = penombre((M - self.position).norm().dot(self.direction)) * np.exp(-abs(self.position - M) * np.log(2) / self.halfLifeDistance)
        return rgb(Coeff * self.color.x, Coeff * self.color.y, Coeff * self.color.z)
        
    @classmethod
    def keys(cls):
        return ['position', 'direction', 'halfAngle', 'color', 'halfLifeDistance', 'penumbraSize']

grenouille = ConicSource(vec3(0,0,0), vec3(0,0,0), 45, rgb(0.2,0.2,0.2))
print(grenouille.serialize())